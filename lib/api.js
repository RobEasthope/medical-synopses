...
export async function getSynopsesWithSlug() {
  const data = await sanityClient.fetch(
    `*[_type == "synopsis"][0]{ 'slug': slug.['tr'].current }`
  );
  return data;
}
...
export async function getSynopsisAndSynopses(slug, preview) {
  ...
  const [synopsis, synopses] = await Promise.all([
    curClient
      .fetch(
        `*[_type == "synopsis" && slug.['tr'].current == $slug] | order(_updatedAt desc) {
        title,
      }`,
        { slug }
      )
      .then(res => res?.[0]),
    curClient.fetch(
      `*[_type == "synopsis" && slug.['tr'].current != $slug] | order(date desc, _updatedAt desc){
        title,
      }[0...2]`,
      { slug }
    ),
